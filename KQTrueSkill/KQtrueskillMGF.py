    ...
    for row in csv_reader:
        if line_count == 0:
            ...
            if playerteam in teams[tournament].keys() :
                print(f'\tteams[{tournament}][{playerteam}] is {teams[tournament][playerteam]}, adding {playername}')
                teams[tournament][playerteam].append(playername)
            else :
                teams[tournament][playerteam] = [playername]
                print(f'\tmade new team for {playerteam}. teams[{tournament}][{playerteam}] is {teams[tournament][playerteam]}')
...
with open('2019 KQ - 2019 game results.csv') as csv_file:
    ...
    for row in csv_reader:
        if line_count == 0:
            ...
            team1wins = int(row[4])
            team2wins = int(row[5])
            #assemble team objects for rating
            t1ratings = []
            t2ratings = []
...
with open('2019PlayerSkill.old.csv', mode='w') as playerskillfile:
    ...
    for player in playerratings.keys() :
        row = [player,playerratings[player].mu,playerratings[player].sigma,len(playertournaments[player]),playergames[player]]
